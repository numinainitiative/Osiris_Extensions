...
namespace Steam
{
    ...
    public class MetadataProvider
    {
        ...
        internal static string GetLibraryCoverUrl(KeyValue productInfo, uint appId, string languageKey)
        {
            var capsule = productInfo?["common"]?["library_assets_full"]?["library_capsule"];
            var assetPath = GetLocalizedAssetPath(capsule?["image2x"], languageKey);
            if (string.IsNullOrEmpty(assetPath))
            {
                assetPath = GetLocalizedAssetPath(capsule?["image"], languageKey);
            }

            if (!string.IsNullOrEmpty(assetPath))
            {
                return $"https://shared.akamai.steamstatic.com/store_item_assets/steam/apps/{appId}/{assetPath.TrimStart('/')}";
            }

            return $"https://steamcdn-a.akamaihd.net/steam/apps/{appId}/library_600x900_2x.jpg";
        }

        private static string GetLocalizedAssetPath(KeyValue assets, string languageKey)
        {
            if (assets == null || assets == KeyValue.Invalid)
            {
                return null;
            }

            var localizedAsset = assets[languageKey];
            if (!string.IsNullOrEmpty(localizedAsset?.Value))
            {
                return localizedAsset.Value;
            }

            var englishAsset = assets["english"];
            if (!string.IsNullOrEmpty(englishAsset?.Value))
            {
                return englishAsset.Value;
            }

            return assets.Children.FirstOrDefault(asset => !string.IsNullOrEmpty(asset.Value))?.Value;
        }
            ...
            bool downloadVerticalCovers)
        {
            ...
            {
                var imageUrl = GetLibraryCoverUrl(productInfo, appId, settings.LanguageKey);
