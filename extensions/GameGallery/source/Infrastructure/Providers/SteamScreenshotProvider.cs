...
namespace SteamScreenshots.Infrastructure.Providers
{
    public class SteamScreenshotProvider : IScreenshotProvider
    {
        ...
        public List<TrailerData> GetTrailers(string id, CancellationToken cancellationToken = default)
        {
            var appDetails = TryGetAppDetails(id, cancellationToken);
            if (appDetails?.data?.Movies.HasItems() != true)
            {
                return new List<TrailerData>();
            }

            return appDetails.data.Movies
